            ...
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    ...
                    else // not an existing user, create a user without roles
                    {
                        ...
                        // add a user and have it return a bool
                        if (usrMgr.AddUser(user))
                        {
                            ...
                            var result = await UserManager.CreateAsync(identityUser, "newuser");
                            if (result.Succeeded)
                            {
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                try
                {
                    if (usrMgr.FindUser(model.Email))
                    {
                        // this requries the user to use the same password as the one in the internal database
                        ...
                        var user = new ApplicationUser
                        {
                            ...
                            // poluate thse fields normally
                            UserName = model.Email,
                            Email = model.Email

                        };

                        //create the user with the Idenetity system UserManager normally
                        var result = await UserManager.CreateAsync(user, model.Password);
                        if (result.Succeeded)
                        {

                            // use the olderUser.Roles list to add the internnally assigned roles to the user
                ...
                {
                    // cratind old user failed, proably because AuthenticatUser failed
                    return View(model);
                }
            }
